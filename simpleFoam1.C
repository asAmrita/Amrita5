...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
 ...
 void myDatawrite(vector force,scalar runTimeValue)
{  
 ofstream myFile;
  myFile.open("test.csv",std::ios::app);

 {
  myFile<<"\t" <<runTimeValue<<"\t"<<force[0]<<"\t" <<force[1]<<"\n";
...
const fvPatchScalarField& pp = p.boundaryField()[inletPatchID]; 
vector force = vector(0, 0, 0);

forAll(normal,i)
{
 force=force+sum(normal*pp[i]);
...
myDatawrite(force,runTime.value());
